...
@TeleOp(name="TestShovel", group="Pushbot")
public class TestShovel extends LinearOpMode {
...
//             firstError = boat.armPotentiometer.getVoltage();
    ...
    public double servoSlow(double shovelServo) {
        ...
        if (servo_angle < 10.0/180.0){
            boat.glyph_aligner.setPosition(.002);
        }
        else if ((boat.glyph_aligner.getPosition() - shovelServo) < - 8.0 / 180.0) { //CONFLICTING CONTROL KEYS
