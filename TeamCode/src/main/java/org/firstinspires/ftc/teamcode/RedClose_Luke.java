...
//            }
        ...
        if(getHeading() > 5){ //Adjusts if robot is too angled left
            drive(0, 2, 1);
        }
        if(getHeading() < -5){ //Adjusts if robot is too angled right
            drive(PI, 2, 1);
        }
        rotate(0);
        autoDetectColumn();
        drive(PI, 7, 1); //Adjusts to make room for rotate
        ...
        /*
        //Moves robot back to balance stone because nathan is a lazy bitch
        rotate(0);
        drive(-PI/2, 16, 1);
        drive(0, 10, 1);
        rotate(0);
    ...
    public void rotate(double angle) {
        ...
        //TURN COUNTER CLOCKWISE
            if (angle - getHeading() > 0 || angle - getHeading() < -180) {
                if(angle - getHeading() > 80){
                    power = 0.6;
                }
                else if(angle - getHeading() > 30){
                    power = 0.2;
                }
                else{
                    power = 0.1;
