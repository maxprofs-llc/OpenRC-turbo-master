...
@TeleOp(name="AdwithTeleop", group="Pushbot")
public class AdwithTeleop extends LinearOpMode {
  ...
  /////////////////////////////////////////////*/
    ...
    public void shovelFlip() {
        ...
        if (gamepad2_a_toggle == true) {
            //toggle
            glyph_flipper_runToPosition(2.40, .3);//old glyph flip high thing //.5
            if (boat.armPotentiometer.getVoltage() <.45){
                boat.glyph_aligner.setPosition(.2);
            }
            else if (boat.armPotentiometer.getVoltage() > .50) {
                boat.glyph_aligner.setPosition(.99); //.99
            }
...
//            boat.right_intake.setPower(-.3);
            glyph_flipper_runToPosition(.25, .50); // OLD ONE FROM FEB 15 THAT WORKS
            ...
            if (boat.armPotentiometer.getVoltage()< 2.45) {
                servoSlow(.46, 4.0); //.25, 10.0 increment
...
//            }
        ...
        if (gamepad2.b == true && runtime.milliseconds() > (glyph_grab_last_press_time + desiredTime)){ //glyph flicking
            ...
            else if (switchangle == .99){
                boat.glyph_aligner.setPosition(.85);
                switchangle = .81;
            }
        ...
        if (gamepad2_x_toggle == true){
            ...
            if (boat.armPotentiometer.getVoltage()< 2.30) {
                servoSlow(.35, 4.0); //.28, 5.5 increment as of 2/10
            }
            boat.conveyor.setPower(1);
            // .15, 9.0 was for hover

            Playglyph_flipper_runToPosition(.20, .9); //.44
            //glyph_flipper_runToPosition(.44, .60);//OLD ONE FROM FEB 15 THAT WORKS//old glyph flip high thing //used to be .24 2/12
